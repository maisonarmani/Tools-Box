...
def execute(filters=None):
    ...
    if filters.get('modified_from') and filters.get('modified_to'):
        conditions += ' and tabAcc.prepared_date  BETWEEN \'{0}\' and \'{1}\''.format(filters.get('modified_from'),
                                                                                      filters.get('modified_to'))
    ...
    sql = '''select tabAcc.name accident,tabAcc.employee_name victim, tabAcc.type_of_accident nature_of_accident ,
		tabAcc.fire_extinguisher_used fire_extinguisher_used,tabFire.location fire_extinguisher_used, tabAcc.prepared_by
		reported_by,tabAcc.prepared_date reported_date from `tabFire Extinguisher` tabFire LEFT JOIN `tabAccident` tabAcc
