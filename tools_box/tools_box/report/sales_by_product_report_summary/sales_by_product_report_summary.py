...
def execute(filters=None):
    columns, data = ["Item:Link/Item:200", "Item Name:Data:200", "Qty:Float:200", "Amount With Discount:Currency:150",
    ...
    data = frappe.db.sql("""SELECT soi.item_code,soi.item_name,sum(soi.qty),sum(soi.net_amount) as net_amount, 
				sum(amount)  amount FROM `tabSales Invoice Item` soi JOIN `tabSales Invoice` si
