...
def execute(filters=None):
	...
	data = frappe.db.sql("""select s.item_code,it.item_name, s.qty_after_transaction,i.warehouse_reorder_level,
			i.warehouse_reorder_qty,it.lead_time_days from `tabStock Ledger Entry` s join (
		...
		join `tabItem` it on (it.item_code=s.item_code) where s.qty_after_transaction < i.warehouse_reorder_level
		and s.warehouse="{0}"
		""".format(filters.get("warehouse")), as_list=1)


	items_with_no_ledgr_record = frappe.db.sql("""select i.parent,it.item_name,"0",i.warehouse_reorder_level,i.warehouse_reorder_qty,it.lead_time_days 
		from `tabItem Reorder` i join `tabItem` it on it.item_code=i.parent where i.warehouse = "{0}" and i.parent 
		NOT IN (select s.item_code from `tabStock Ledger Entry` s group by s.item_code)"""
						  .format(filters.get("warehouse")), as_list=1)
	for x in items_with_no_ledgr_record:
		data.append(x)
	return columns, data
