...
def execute(filters=None):
    columns, data = ["Date:Date:200", "Production Order:Link/Production Order:150", "Item Code:Link/Item:100",
                     "Item:Data:200", "UOM:Link/UOM:75","Qty:Float:100", "Transferred by:Data:100", "Received By:Data:150"], []
    # Date	Item	UOM	Qty
    conditions = ""
    if filters.get("item"):
    ...
    data = frappe.db.sql("""SELECT f.date, p.name, p.expected_delivery_date, i.item_code,i.item_name,i.uom,i.qty, 
        f.transfered_by_name, f.received_by_name from `tabFinished Goods Transfer Item` i JOIN `tabFinished Goods Transfer Form` f 
